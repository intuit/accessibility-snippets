...
**CommentAria** - Add `role=mark` to mark the content in which the comment is in reference to and add `aria-details` containing ID of comment. Add `role=comment` to a comment or reaction to some content on the page and add `id` to indicate comment ID. 
